  ...
  <div class="system-config">
    ...
    <Table ref="table" width="550" border :columns="columns" :data="roleList">
      <template slot-scope="props" slot="action">
        <Button
          @click.native="modify(props.rowInfo)"
          type="text"
          class="fix-table-btn pointer"
          target="_blank"
...
export default {
  ...
  methods: {
    ...
    modify(row) {
      ...
      this.treeList = this.fillChecked(this.treeList,checkPages)
      this.roleId = row.id
      this.formData.name = row.name
      this.showModal = true
    },
    fillChecked(data,checkPages){
      let resData = []
      data.forEach(item => {
        let oItem = Object.assign({},item)
        if(checkPages.includes(String(item.id))){
          oItem.checked = true
          oItem.expand = true
        }
        if (item.children && item.children.length) {
          oItem.children = this.fillChecked(oItem.children,checkPages)
        }
        resData.push(oItem)
      })
      return resData
    ...
    getAllMenuList() {
      this.$store.dispatch('getAllMenuList').then(res => {
        let list = res.data || []
        this.treeList =
          creatTreeData(Router, list, this.$store.state.actionMap) || []
      })
    }
  },
